			...
			<div className={styles.image_n_features}>
				...
				<div className={styles.features}>
					...
					<div className={styles.lvl_container}>
						<div>{lg === 'ru' ? 'Уровень: ' : 'lvl: '} {newLevel && `${card.lvl - 1} > `}<span className={newLevel && styles.new_lvl}>{card.lvl}</span></div>
						{isMyMonster && <div className={styles.xp}>{lg === 'ru' ? 'Опыт: ' : 'xp: '} {card.xp}/{card.lvl*100}</div>}
					</div>
